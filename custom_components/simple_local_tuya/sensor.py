...
class TuyaLocalSensor(CoordinatorEntity, SensorEntity):
    ...
    def extra_state_attributes(self):
        """每次 coordinator 数据更新时刷新此属性，确保 HA 触发 state_changed 事件"""
        attrs = {}
    ...
    def native_value(self):
        ...
        raw = data.get(self._dps_id)
        if raw is None:
            return self._last_value

        # 标记本次更新时间（即使值不变，extra_state_attributes 也会变）
        self._last_updated = datetime.now()

        try:
            if self._config.get("signed"):
                raw = self._get_signed_value(raw)
            value = float(raw) * self._config.get("scaling", 1)
